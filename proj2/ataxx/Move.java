...
class Move {
    ...
    boolean isExtend() {
        ...
        cOffset = _col0 - _col1;
        rOffset = _row0 - _row1;
        return cOffset <= 1 && cOffset >= -1 && rOffset <= 1 && rOffset >= -1;
    }

    /** Return true if thIs is a jump (move to adjacent square). */
    boolean isJump() {
        return !isPass() && !isExtend();
    }
    ...
    public String toString() {
        if (isPass()) {
            return "-";
        } else if (isExtend() || isJump()) {
            String s = Character.toString(col0()) + Character.toString(row0())
                + "-" + Character.toString(col1()) + Character.toString(row1());
            return s;
        } else {
            throw new GameException("Cannot convert this move to string.");
        }
    }
